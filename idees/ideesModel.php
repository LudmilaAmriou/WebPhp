...
class ideesModel extends tempModel {
    ...
    public function recherche($brand,$ram,$note,$cuiss,$prep,$total,$kcal){
        ...
        if(isset($_POST["action"])|| isset($_GET['cat'])){
            ...
            JOIN ingredient ON ingredient.recette_id = subquery.recette_id WHERE notation>3 AND saisonIng IN ('hiver')";
            if(isset($brand))
            {
            ...
            if(isset($ram))
            {
                ...
                $sql .= " OR saisonIng IN ('".$ram_filter."')";
            }
    ...
    public function ingred(){
        $DB_con = parent::connexion($this->DB_host,$this->DB_name,$this->DB_pass,$this->DB_user);       
            $sql = "SELECT nomIng,any_value(recette_id) AS recette_id FROM ingredient GROUP BY nomIng";
        try {
            $query = $DB_con->prepare($sql);
        } catch (Exception $e) {
            echo $e->getMessage();
    ...
    public function ing($recette){
        $DB_con = parent::connexion($this->DB_host,$this->DB_name,$this->DB_pass,$this->DB_user);     
            $sql = "SELECT * FROM ingredient WHERE recette_id = $recette";
        try {
        $query = $DB_con->prepare($sql);
        } catch (Exception $e) {
            echo $e->getMessage();
